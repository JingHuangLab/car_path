...
# Fine convergence criterion for dihedral rotations before CI.
...
def mk_chain(rxn_id: int) -> Tuple[ChainCart, List]:
    ...
    if rxn_id in [1, 3, 11, 14, 15, 21, 23]:
...
def exec_cos(chain: ChainCart, atom_names, work_dir: str):
    ...
    chain.set_chain_vec(numpy.load(f'{work_dir}/mini_finl_fine.npy'))
    cos = CARPT(chain, verbose=False, )
    opt = ConsGradientDescent(cos, gd_eta=.003, fix_mode='both')
    convergence_criteria = 0.0001 if 'rxn11' in work_dir else 0.0007 if 'rxn21' in work_dir else 0.007 # if rxn17
    # number of atoms.
    ...
    # loggers.
    # open(f'{work_dir}/mini_traj_fine.xyz', 'w').close() # clean up previous traj.
    run_logger = open(f'{work_dir}/mini_ener_fine.log', 'a')
    xyz_logger = open(f'{work_dir}/mini_traj_fine.xyz', 'a')

    # Make initial trajectory.
    xyz_logger.write(f'{n_atoms*7}\nStep    0\n')
    for irep in range(7):
        rep_vec = pv_cvec[irep, :]
        for i_atom in range(n_atoms):
            xyz_logger.write(f'{atom_names[i_atom]:<8}{rep_vec[i_atom*3]:<16.8f}{rep_vec[i_atom*3+1]:<16.8f}{rep_vec[i_atom*3+2]:<16.8f}\n')
    xyz_logger.flush()
    ...
    for istep in range(1, 501):
        ...
        if current_diffr_ene <= convergence_criteria: # Convergence criteria.
            numpy.save(f'{work_dir}/mini_finl_fine.npy', chain.get_chain_vec())
            ...
            if numpy.sum(opt_current_state[1]) <= numpy.sum(opt_previous_state[1]): 
                ...
                if numpy.max(opt_current_state[1]) > numpy.max(opt_previous_state[1]):
                    ...
                    step_rejected = True if not istep == 18 else False
                    opt._gd_eta *= .5
        ...
        if step_rejected == False:
        # Make trajectory.
            xyz_logger.write(f'{n_atoms*7}\nStep {istep:>4}\n')
            for irep in range(7):
                rep_vec = opt_current_state[0][irep, :]
                for i_atom in range(n_atoms):
                    xyz_logger.write(f'{atom_names[i_atom]:<8}{rep_vec[i_atom*3]:<16.8f}{rep_vec[i_atom*3+1]:<16.8f}{rep_vec[i_atom*3+2]:<16.8f}\n')
            xyz_logger.flush()

    write_xyz_traj(f'{work_dir}/mini_finl_fine.xyz', chain.get_chain_vec(), atom_names)
